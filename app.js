(function () {
  ...
  function renderParent(){
    ...
    body.innerHTML=`<div class="reportGrid">${d.subjects.map(s=>`<article class="reportCard"><div class="subjectHead"><h3>${s.icon} ${s.name}</h3><span>${fmt(s.today)}</span></div><div class="big">${fmt(s.week)}</div><div class="sub">最近7天有效学习时间</div><div class="metric"><span>学习成果</span><b>${s.main}</b></div><div class="metric"><span>学习档案</span><b>${s.extra}</b></div><div class="metric"><span>建议关注</span><b>${s.next}</b></div><a href="${s.url}">进入${s.name}详细后台 →</a></article>`).join("")}</div>
      ...
      <div class="archive backupCenter"><div class="archiveHead"><div><b>🛟 三科数据保险箱</b><div class="sub">${health.ok}/${health.total} 份核心存档正常 · ${(health.bytes/1024).toFixed(1)}KB · ${health.last?(health.days===0?"今天已备份":`${health.days}天前备份`):"还没有做过三科总备份"}</div></div></div><div class="backupActions"><button class="parentBtn" id="downloadBackup">保存三科备份文件</button><button class="parentBtn ghost" id="copyBackup">复制三科备份码</button><button class="parentBtn ghost" id="openRestore">恢复旧备份</button></div><div class="restoreBox" id="restoreBox" hidden><label class="backupFile">选择备份文件<input id="restoreFile" type="file" accept="application/json,.json"></label><span>或者粘贴备份码</span><textarea id="restoreCode" placeholder="点击这里，再粘贴三科备份码" autocomplete="off"></textarea><button class="parentBtn" id="restoreNow">确认恢复三科进度</button><small id="restoreMsg"></small></div><div class="parentAdvice">${health.days===null||health.days>=7?"建议现在保存一次；以后每周或换设备前再保存。":"备份状态正常。清缓存、换手机或系统升级前再保存一次。"} 备份不包含AI口令和家长密码。</div></div>`;
    body.querySelectorAll("[data-coin]").forEach(b=>b.onclick=()=>adjustWallet(Number(b.dataset.coin),0));body.querySelectorAll("[data-ticket]").forEach(b=>b.onclick=()=>adjustWallet(0,Number(b.dataset.ticket)));$("#saveReport").onclick=saveReportImage;$("#downloadBackup").onclick=downloadBackup;$("#copyBackup").onclick=copyBackup;$("#openRestore").onclick=()=>{$("#restoreBox").hidden=!$("#restoreBox").hidden};$("#restoreFile").onchange=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{$("#restoreCode").value=backupCode(String(r.result||""));$("#restoreMsg").textContent="备份文件已读取，点击确认恢复"};r.readAsText(f)};$("#restoreNow").onclick=()=>{const msg=$("#restoreMsg"),code=$("#restoreCode").value.trim();if(!code){msg.textContent="请先选择备份文件或粘贴备份码";return}if(restoreBackup(code)){msg.textContent="恢复成功，重新打开三个学科即可看到进度";paint()}else msg.textContent="备份码不完整或已损坏，请重新复制"};
  }
  ...
  function copyBackup(){const code=backupCode(makeBackup()),done=()=>{markBackedUp();renderParent()};if(navigator.clipboard?.writeText)navigator.clipboard.writeText(code).then(done).catch(()=>fallbackCopy(code,done));else fallbackCopy(code,done)}
  ...
  window.learningHub = { snapshot, paint, todayStr, journeyData, makeBackup, backupCode, restoreBackup };
}());
